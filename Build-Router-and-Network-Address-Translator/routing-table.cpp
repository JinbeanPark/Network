...
RoutingTable::lookup(uint32_t ip) const
{
    ...
    // Sorting routing table entries by mask
    std::list<RoutingTableEntry> sortedEntries = m_entries;
    sortedEntries.sort([](const RoutingTableEntry & e1, const RoutingTableEntry & e2) { return e1.mask > e2.mask;});

    // Finding the longest prefix match from a longest prefix
    std::list<RoutingTableEntry>::const_iterator i;
    for (i = sortedEntries.begin(); i != sortedEntries.end(); i++) {
